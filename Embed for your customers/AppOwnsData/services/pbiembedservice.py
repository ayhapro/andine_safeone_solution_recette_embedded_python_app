...
class PbiEmbedService:
    ...
    def get_embed_params_for_multiple_reports(self, workspace_id, report_ids, additional_dataset_ids=None):
        ...
        for report_id in report_ids:
            ...
            dataset_ids.add(api_response['datasetId'])

        # Append additional datasets to the set to achieve dynamic binding later
        if additional_dataset_ids is not None:
            dataset_ids.update(additional_dataset_ids)

        embed_token = self.get_embed_token_for_multiple_reports_single_workspace(report_ids, list(dataset_ids), workspace_id)
